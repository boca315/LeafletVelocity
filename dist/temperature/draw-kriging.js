var TemperatureOverlay = L.Layer.extend({
    ...
    initialize: function (colors) {
        ...
        // this.cfg = config;
        this.colors = colors;
        // this.bounds = bounds; // 边界线
        this.tempture = []; // 温度
        this.idwpage = [];// 画出来的气温图
    },

    onAdd: function (map) {
        this._map = map;
        // 一开始画数据
        this._draw(this.tempture, this.colors);
    },
    ...
    onRemove: function (map) {
        // 将数据清空
        map.removeLayer(this.idwPage);
    },

    _draw: function (tempture, colors) {
        // 根据坐标和值画图
        var positions = [];
        bounds[0].forEach(function (mypoint) {
            positions.push([mypoint[1], mypoint[0]]);
        });
        //  var scope = L.polyline(positions, { color: 'red' }).addTo(map);
        positions = [];
        bounds[1].forEach(function (mypoint) {
            positions.push([mypoint[1], mypoint[0]]);
        });
        var scope = L.polyline(positions, { color: 'red' });
        this._map.fitBounds(scope.getBounds());

        // var southWest = scope._bounds._southWest; //  {lat: 18.105500000000234, lng: 73.47660000000019}
        // var northEast = scope._bounds._northEast; // {lat: 53.56930000000017, lng: 135.08790000000022}
        var xlim = [73.47660000000019, 135.08790000000022];
        var ylim = [18.105500000000234, 53.56930000000017];
        // [73.4766, 135.088], [18.1055, 53.5693]
        ...
        kriging.plot(canvas, grid, [73.4766, 135.0879], [18.1055, 53.5693], colors);
        // }
        ...
        var idwPage = L.imageOverlay(returnImgae(), imageBounds, { opacity: 0.9 });
        this.idwPage = idwPage;
        this._map.addLayer(idwPage);
        //idwPage.addTo(map);
    },
     ...
     * @param {数据} data 
     */
    addData: function (tempture) {
        this.tempture.push(tempture);
        this._draw(data);
    }
});